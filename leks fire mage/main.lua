...
local menu = {
    ...
    enable_rotation = core.menu.checkbox(true, "fire_mage_enable_rotation_cb"),
    use_combustion = core.menu.checkbox(true, "fire_mage_use_combustion_cb"),
    use_shifting_power = core.menu.checkbox(true, "fire_mage_use_shifting_power_cb"),
    ...
    use_phoenix_flames = core.menu.checkbox(true, "fire_mage_use_phoenix_flames"),
    use_meteor = core.menu.checkbox(true, "fire_mage_use_meteor"),
    meteor_min_targets = core.menu.slider_int(2, 10, 3, "fire_mage_meteor_min_targets"),
    use_blazing_barrier = core.menu.checkbox(true, "fire_mage_use_blazing_barrier"),
    ...
    use_arcane_intellect = core.menu.checkbox(true, "fire_mage_use_arcane_intellect"),
...
local function on_render_menu()
    menu.main_node:render("Lek's Fire Mage", function()
        ...
        menu.use_combustion:render("Auto-Use Combustion")
        menu.use_shifting_power:render("Auto-Use Shifting Power")
        ...
        if menu.use_blazing_barrier:get_state() then
            menu.blazing_barrier_min_health_pct:render("Min Health % for Barrier (Current)", "Casts Blazing Barrier if current health falls below this percentage.")
            menu.blazing_barrier_min_health_inc_pct:render("Min Health % for Barrier (Predicted)", "Casts Blazing Barrier if predicted health (after incoming damage) falls below this percentage.")
        end
        ...
        menu.ftb_refresh_threshold:render("Feel the Burn Refresh (ms)", "How early (in milliseconds) to refresh Feel the Burn before it expires.")
    end)
end
...
local function get_primary_rotation_target(player, nearby_enemies, consider_cleave)
    ...
    if player_target and unit_helper:is_valid_enemy(player_target) then
...
local function should_cast_meteor(player, nearby_enemies, use_aoe_rotation, primary_target)
    ...
    local meteor_cd = core.spell_book.get_spell_cooldown(SPELL.METEOR)
    if meteor_cd > 0 then return false, nil end
...
local function main_logic()
    if not menu.enable_rotation:get_state() then return end
    ...
    elseif menu.use_combustion:get_state() and combustion_cd == 0 and fire_blast_charges > 0 then
    ...
    elseif menu.use_shifting_power:get_state() and fire_blast_charges == 0 and phoenix_flames_charges == 0 and not is_moving and spell_helper:is_spell_castable(SPELL.SHIFTING_POWER, player, player) then
